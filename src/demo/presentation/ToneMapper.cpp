...
std::expected<ToneMapper, VkResult> ToneMapper::create(const DeviceContext& ctx,
    ...
    const VkResult result = vkCreateGraphicsPipelines(
        ctx.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &mapper.m_pipeline);
    ...
    ctx.setDebugName(VK_OBJECT_TYPE_PIPELINE,
                     reinterpret_cast<uint64_t>(mapper.m_pipeline),
                     "hyperion.tonemapPipeline");
    return mapper;
}
...
void ToneMapper::record(VkCommandBuffer cmd,
    ...
    vkCmdPushDescriptorSet(
        cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelineLayout, 0, 1, &hdrWrite);
